...
(function () {
  ...
  // Dark paths must not leave the snippet's bare stub array on window: it is truthy but has no .capture, so every
  // `if(window.posthog) posthog.capture(...)` threw and card clicks died for opted-out browsers (2026-09-25).
  function goDark() { window.__phDisabled = true; window.posthog = undefined; }
  if (KEY.indexOf("YOUR_") === 0) { goDark(); return; } // not configured yet: stay dark
  ...
  try {
    ...
    if (localStorage.getItem("ph_optout") === "1") { goDark(); return; }
  } catch (e) {}
  posthog.init(KEY, {
